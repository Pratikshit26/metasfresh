...
describe("Create test data", function () {
  it("Read fixture and prepare the names", function () {
    cy.fixture("returns/create_customer_return_from_shipment.json").then(
      (f) => {
  ...
  it("Create Sales Order", function () {
    new SalesOrder()
      ...
      .addLine(
        new SalesOrderLine().setProduct(productName).setQuantity(productQty),
      )
      .apply();
    cy.completeDocument();
...
describe("Generate the Shipment", function () {
  ...
  it("Shipment checks", function () {
    ...
    cy.expectNumberOfRows(1);
    cy.selectRowByColumnAndValue({
      column: shipmentHandlingUnitsColumn,
      value: huValue,
    });
...
describe("Create Customer return from Shipment", function () {
  it("Go to Shipment and create customer return", function () {
    ...
    cy.selectRowByColumnAndValue({
      column: huSelectionHuCodeColumn,
      value: huValue,
    });
  ...
  it("Check the Customer Return", function () {
    ...
    cy.getStringFieldValue("C_DocType_ID").should(
      "equals",
      expectedCustomerReturnDocType,
    );
