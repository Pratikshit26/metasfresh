...
export class Inventory {
  ...
  static applyInventory(newInventory) {
    ...
    cy.getStringFieldValue("C_DocType_ID").then((currentDocTypeName) => {
      if (currentDocTypeName !== newInventory.docTypeName) {
        cy.log(
          `Change C_DocType_ID from ${currentDocTypeName} to ${newInventory.docTypeName}`,
        );
        cy.selectInListField(
          "C_DocType_ID",
          newInventory.docTypeName,
          false,
          null,
          true,
        );
    ...
    cy.fixture("inventory/inventory.json").then((inventoryJson) => {
      if (
        this.docTypeName ==
        getLanguageSpecific(inventoryJson, "aggregatedHUsInventoryDocTypeName")
      ) {
  ...
  static applyInventoryLine(inventoryLine) {
    ...
    cy.getStringFieldValue("HUAggregationType", true /*modal*/).then(
      (huAggregationType) => {
        cy.fixture("inventory/inventory.json").then((inventoryJson) => {
          if (
            this.docTypeName ==
