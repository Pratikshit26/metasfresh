...
test("Distribution and manufacturing test", async ({ page }) => {
  ...
  await DistributionJobsListScreen.filterByFacetId({
    facetId: warehouseFrom1FacetId,
    expectHitCount: 2,
  });
  await DistributionJobsListScreen.startJob({
    launcherTestId: launcherTest1Id,
  });
  await DistributionJobScreen.clickLineButton({ index: 1 });
  await DistributionLineScreen.scanHUToMove({
    huQRCode: comp1_huQRCode,
    expectedQtyToMove: "5",
  });
  await DistributionLineScreen.clickStepButton({ index: 1 });
  await DistributionStepScreen.scanDropToLocator({
    dropToLocatorQRCode: dropToLocatorQRCode,
  });
  ...
  await DistributionJobsListScreen.startJob({
    launcherTestId: launcherTest2Id,
  });
  await DistributionJobScreen.clickLineButton({ index: 1 });
  await DistributionLineScreen.scanHUToMove({
    huQRCode: comp2_huQRCode,
