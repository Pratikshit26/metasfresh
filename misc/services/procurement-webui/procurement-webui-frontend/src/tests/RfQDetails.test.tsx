...
test('renders RfQDetails component correctly', () => {
    const wrapper = render(
        <MemoryRouter initialEntries={['/rfq/15']}>
            <Provider store={store}>
                <RootStoreContext.Provider value={store}>
                    <Route exact path="/rfq/:quotationId" component={RfQDetails} />
    ...
    expect(html).toContain('{RfQView.ProductName}');
    expect(html).toContain('Frisée Industrie');
    expect(html).toContain('RfQView.ProductPackingInfo');
    expect(html).toContain('G1 x 10 kg');
    expect(html).toContain('01/09/2021');
    expect(html).toContain('RfQView.DateClose');
    expect(html).toContain('31/07/2021');
    expect(html).toContain('RfQView.QtyPromised');
    expect(html).toContain('27 kg');
    expect(html).toContain('RfQView.Price');
    expect(html).toContain('1,2 EUR');
    expect(html).toContain('RfQView.DailyQuantities');
