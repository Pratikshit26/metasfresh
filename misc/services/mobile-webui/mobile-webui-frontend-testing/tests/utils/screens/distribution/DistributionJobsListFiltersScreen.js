...
export const DistributionJobsListFiltersScreen = {
  waitForScreen: async () =>
    await test.step("Wait for distribution jobs list filters screen", async () => {
      await page.locator("#WFLaunchersFiltersScreen").waitFor();
      await DistributionJobsListFiltersScreen.waitLoadComplete();
    }),

  waitLoadComplete: async () =>
    await test.step("Wait for load complete", async () => {
      await page
        .locator(".loading")
        .waitFor({ state: "detached", timeout: SLOW_ACTION_TIMEOUT });
    }),

  filterByFacetId: async ({ facetId, expectHitCount }) =>
    await test.step(`Filter by facet ${facetId}`, async () => {
      const facetButton = page.locator(`[data-testid="${facetId}"]`);
      if (expectHitCount != null) {
        await expect(facetButton).toHaveAttribute(
          "data-hitcount",
          String(expectHitCount),
        );
      }
