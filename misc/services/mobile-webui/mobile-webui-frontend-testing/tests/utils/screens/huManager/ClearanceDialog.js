...
export const ClearanceDialog = {
  waitForDialog: async () =>
    await test.step(`${NAME} - Wait to open`, async () => {
      await containerElement().waitFor();
    }),

  waitForDialogToClose: async () =>
  ...
  expectVisible: async () =>
    await test.step(`${NAME} - Expect to be displayed`, async () => {
      await expect(containerElement()).toBeVisible();
    }),

  clickClearanceStatus: async (status) =>
    await test.step(`${NAME} - Click on Clearance "${status}"`, async () => {
      await ClearanceDialog.expectVisible();
      await page
        .getByTestId(`clearanceStatus-${status}`)
        .tap({ timeout: FAST_ACTION_TIMEOUT });
    }),

  typeNote: async (note) =>
    await test.step(`${NAME} - Type note: ${note}`, async () => {
      await ClearanceDialog.expectVisible();
      await page.getByTestId("clearanceNote-input").type(note);
    }),

  clickOK: async () =>
    await test.step(`${NAME} - Click OK`, async () => {
      await ClearanceDialog.expectVisible();
