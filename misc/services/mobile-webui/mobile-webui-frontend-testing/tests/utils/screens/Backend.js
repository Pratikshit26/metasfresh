...
export const Backend = {
  createMasterdata: async ({ authToken, language, request }) =>
    await test.step(`Backend: create master data`, async () => {
      ...
      console.log(
        `Created master data (${language}):\n` +
          JSON.stringify(responseBody, null, 2),
      );

      if (responseBody.errors || responseBody.stackTrace) {
        throw Error(
          "Got error while creating master data:\n" +
            JSON.stringify(responseBody, null, 2),
        );
      }
  ...
  getFreePickingSlot: async ({ bpartnerCode } = {}) =>
    await test.step(`Backend: get free picking slot`, async () => {
      const backendBaseUrl = await getBackendBaseUrl();
      const request = { bpartnerCode };
      console.log(`Sending request":\n` + JSON.stringify(request, null, 2));
      const response = await page.request.post(
        `${backendBaseUrl}/frontendTesting/getFreePickingSlot`,
        {
          data: request,
          headers: {
            "Content-Type": "application/json",
...
let _backendBaseUrl = process.env.BACKEND_BASE_URL
  ? process.env.BACKEND_BASE_URL + "/api/v2"
  : null;
...
export const loadConfigFromFrontendApp = async () =>
  await test.step(`Fetching from mobile-webui-frontend/public/config.js`, async () => {
    ...
    const serverUrlRef = await page.waitForFunction(
      () => window.config?.SERVER_URL,
    );
    const serverUrl = await serverUrlRef.jsonValue();
    if (!serverUrl) {
      throw new Error(
        "window.config.SERVER_URL is not defined in the frontend app. " +
