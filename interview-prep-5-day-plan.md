# 5-Day Interview Preparation Plan

**Duration:** January 6-10, 2026

---

## Day 1 - Monday, January 6: Data Structures & Arrays

### Morning (9:00 AM - 12:00 PM)

**Theory Review (1.5 hours)**

- [ ] Arrays fundamentals
- [ ] Two pointers technique
- [ ] Sliding window pattern
- [ ] Array manipulation complexities

**Coding Practice (1.5 hours)**

- [ ] Two Sum (Easy)
- [ ] Best Time to Buy and Sell Stock (Easy)
- [ ] Container With Most Water (Medium)
- [ ] Product of Array Except Self (Medium)
- [ ] Trapping Rain Water (Hard)

### Afternoon (2:00 PM - 5:00 PM)

**Strings & HashMaps (2 hours)**

- [ ] String manipulation techniques
- [ ] HashMap/HashSet usage patterns
- [ ] Character frequency problems

**Practice Problems (1 hour)**

- [ ] Valid Anagram (Easy)
- [ ] Group Anagrams (Medium)
- [ ] Longest Substring Without Repeating Characters (Medium)
- [ ] Minimum Window Substring (Hard)

### Evening (7:00 PM - 9:00 PM)

**Mock Interview Practice**

- [ ] Record yourself solving 2 medium problems
- [ ] Review and critique your approach
- [ ] Note communication gaps

**Resources:**

- LeetCode: Arrays & Strings tags
- NeetCode 150 roadmap
- AlgoExpert videos

---

## Day 2 - Tuesday, January 7: Linked Lists & Trees

### Morning (9:00 AM - 12:00 PM)

**Linked Lists (1.5 hours)**

- [ ] Singly vs Doubly linked lists
- [ ] Fast & slow pointer technique
- [ ] Reversal patterns
- [ ] Cycle detection

**Practice (1.5 hours)**

- [ ] Reverse Linked List (Easy)
- [ ] Merge Two Sorted Lists (Easy)
- [ ] Linked List Cycle (Easy)
- [ ] Remove Nth Node From End (Medium)
- [ ] Reorder List (Medium)

### Afternoon (2:00 PM - 5:00 PM)

**Binary Trees (2 hours)**

- [ ] Tree traversals (Inorder, Preorder, Postorder, Level-order)
- [ ] Recursion patterns
- [ ] DFS vs BFS
- [ ] Binary Search Tree properties

**Practice (1 hour)**

- [ ] Maximum Depth of Binary Tree (Easy)
- [ ] Same Tree (Easy)
- [ ] Invert Binary Tree (Easy)
- [ ] Validate BST (Medium)
- [ ] Binary Tree Level Order Traversal (Medium)

### Evening (7:00 PM - 9:00 PM)

**Advanced Tree Problems**

- [ ] Lowest Common Ancestor (Medium)
- [ ] Serialize and Deserialize Binary Tree (Hard)
- [ ] Review all tree patterns

---

## Day 3 - Wednesday, January 8: Dynamic Programming & Graphs

### Morning (9:00 AM - 12:00 PM)

**Dynamic Programming Fundamentals (2 hours)**

- [ ] Memoization vs Tabulation
- [ ] 1D DP patterns
- [ ] 2D DP patterns
- [ ] Common DP problems classification

**Practice (1 hour)**

- [ ] Climbing Stairs (Easy)
- [ ] House Robber (Medium)
- [ ] Coin Change (Medium)
- [ ] Longest Increasing Subsequence (Medium)

### Afternoon (2:00 PM - 5:00 PM)

**Graph Theory (2 hours)**

- [ ] Graph representations (Adjacency list/matrix)
- [ ] DFS and BFS traversal
- [ ] Topological sort
- [ ] Union-Find

**Practice (1 hour)**

- [ ] Number of Islands (Medium)
- [ ] Clone Graph (Medium)
- [ ] Course Schedule (Medium)
- [ ] Pacific Atlantic Water Flow (Medium)

### Evening (7:00 PM - 9:00 PM)

**Advanced DP**

- [ ] Longest Common Subsequence (Medium)
- [ ] Word Break (Medium)
- [ ] Unique Paths (Medium)
- [ ] Review DP patterns

---

## Day 4 - Thursday, January 9: System Design

### Morning (9:00 AM - 12:00 PM)

**System Design Fundamentals**

- [ ] Scalability principles
- [ ] Load balancing
- [ ] Caching strategies (Redis, CDN)
- [ ] Database sharding & replication
- [ ] CAP theorem

**Case Study Review**

- [ ] Design URL Shortener
- [ ] Design Twitter/Social Feed
- [ ] Design Rate Limiter

### Afternoon (2:00 PM - 5:00 PM)

**Deep Dive Topics**

- [ ] Microservices architecture
- [ ] Message queues (Kafka, RabbitMQ)
- [ ] API design (REST vs GraphQL)
- [ ] Database selection (SQL vs NoSQL)
- [ ] Monitoring & observability

**Design Practice**

- [ ] Design Instagram
- [ ] Design YouTube
- [ ] Design Uber

### Evening (7:00 PM - 9:00 PM)

**System Design for Your Projects**

- [ ] Design architecture for FormBuilder app
- [ ] Document scaling strategy
- [ ] Identify bottlenecks
- [ ] Cost optimization plan

**Resources:**

- System Design Primer (GitHub)
- Grokking the System Design Interview
- ByteByteGo videos
- Your existing system-design-learning-guide.md

---

## Day 5 - Friday, January 10: Behavioral & Interviews

### Morning (9:00 AM - 12:00 PM)

**Behavioral Preparation**

- [ ] STAR method review
- [ ] Prepare 10 stories covering:
  - Leadership/influence
  - Conflict resolution
  - Technical challenges
  - Failure/learning moments
  - Innovation/initiative

**Common Questions Practice**

- [ ] "Tell me about yourself"
- [ ] "Why this company?"
- [ ] "Biggest technical challenge?"
- [ ] "Disagreement with team member?"
- [ ] "Where do you see yourself in 5 years?"

### Afternoon (2:00 PM - 5:00 PM)

**Full Interview #1**

- [ ] 45-minute coding interview (2 problems)
- [ ] Record and review
- [ ] Identify improvement areas

**Company Research**

- [ ] Research target companies
- [ ] Prepare company-specific questions
- [ ] Review their tech stack
- [ ] Read engineering blogs

### Evening (6:00 PM - 9:00 PM)

**Full Interview #2**

- [ ] 60-minute system design interview
- [ ] Practice whiteboarding
- [ ] Clarifying requirements
- [ ] Discussing trade-offs

**Final Prep**

- [ ] Review your resume projects
- [ ] Prepare questions to ask interviewer
- [ ] Review metasfresh and FormBuilder projects in depth
- [ ] Practice explaining technical decisions

---

## Daily Habits (All 5 Days)

### Morning Routine

- [ ] 30-minute exercise/walk (brain oxygenation)
- [ ] Healthy breakfast
- [ ] Review previous day's notes

### During Study Sessions

- [ ] Pomodoro technique (25 min focus, 5 min break)
- [ ] Take handwritten notes
- [ ] Explain concepts out loud
- [ ] Draw diagrams for complex topics

### Evening Routine

- [ ] Summarize day's learnings
- [ ] List 3 things learned
- [ ] Identify weak areas
- [ ] Plan next day's priorities
- [ ] 7-8 hours sleep

---

## Key Resources

### Coding Practice

- **LeetCode Premium** - Company-specific questions
- **NeetCode.io** - Structured roadmap with video explanations
- **AlgoExpert** - Clear video explanations
- **Pramp/Interviewing.io** - Live interviews

### System Design

- **System Design Primer** - GitHub repository
- **Designing Data-Intensive Applications** - Book
- **ByteByteGo** - Visual system design
- **Grokking System Design** - Course

### Behavioral

- **STAR Method Template** - Document your stories
- **Company Glassdoor Reviews** - Real interview questions
- **Levels.fyi** - Compensation data

---

## Success Metrics

### By End of Day 5:

- [ ] Solved 50+ LeetCode problems (20 Easy, 25 Medium, 5 Hard)
- [ ] Completed 5+ system design case studies
- [ ] Conducted 3+ full interviews
- [ ] Prepared 10 STAR method stories
- [ ] Documented FormBuilder & metasfresh architecture clearly
- [ ] Confident explaining all resume projects

---

## Interview Day Checklist

### Day Before

- [ ] Light review only (no new topics)
- [ ] Prepare clothes
- [ ] Test video/audio setup
- [ ] Early bedtime

### Interview Day

- [ ] Arrive/login 10 minutes early
- [ ] Have water nearby
- [ ] Paper and pen ready
- [ ] Positive mindset
- [ ] Remember: They want you to succeed!

---

## Notes

- **Don't panic if stuck** - Think out loud, ask clarifying questions
- **Communication is key** - Explain your thought process
- **Test your code** - Walk through with examples
- **Ask questions** - Shows engagement and critical thinking
- **Be yourself** - Authenticity matters

**You got this! 💪**
