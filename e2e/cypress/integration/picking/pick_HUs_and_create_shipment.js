...
const includedQAUrl = new RegExp(
  /rest\/api\/documentView\/pickingSlot\/.*\/quickActions/,
);
const includedQAUrl2 = new RegExp(
  /rest\/api\/documentView\/husToPick\/.*\/quickActions/,
);
...
describe("Create test data", function () {
  ...
  it("Create first single-HU inventory doc", function () {
    Builder.createHUWithStock(productName, productQty, locatorId).then(
...
describe("Pick the SO", function () {
  ...
  it("Pick first HU", function () {
    ...
    cy.selectLeftTable().within(() => {
      cy.selectRowByColumnAndValue(
        { column: orderColumn, value: soDocNumber },
        false,
        true,
      );
    });
    ...
    cy.executeQuickActionWithRightSideTable(
      "WEBUI_Picking_HUEditor_Launcher",
      true,
    );
    cy.wait("@huQA2", { timeout: 10000 });

    cy.selectRightTable().within(() => {
      cy.selectItemUsingBarcodeFilter(
        { column: huSelectionHuCodeColumn, value: huValue1 },
        false,
        true,
      );
    });
  ...
  it("Pick second HU", function () {
    ...
    cy.selectRightTable().within(() => {
      cy.selectItemUsingBarcodeFilter(
        { column: huSelectionHuCodeColumn, value: huValue2 },
        false,
        true,
      );
    });
  ...
  it("Confirm Picks", function () {
    ...
    cy.selectRightTable().within(() => {
      ...
      cy.selectRowByColumnAndValue(
        { column: pickingHuCodeColumn, value: huValue2 },
        false,
        true,
      );
    });
    ...
    cy.executeQuickAction(
      "WEBUI_Picking_M_Picking_Candidate_Process",
      true,
      false,
    );
    ...
    cy.selectRightTable().within(() => {
      cy.get(".table-row", { timeout: 10000 }).should("exist");
      cy.selectRowByColumnAndValue(
        { column: pickingHuCodeColumn, value: huValue1 },
        false,
        true,
      );
    });
...
describe("Generate the Shipment", function () {
  ...
  it("Shipment checks", function () {
    ...
    cy.selectRowByColumnAndValue({
      column: handlingUnitsShipmentColumn,
      value: huValue1,
    });
    cy.selectRowByColumnAndValue({
      column: handlingUnitsShipmentColumn,
