...
export const ChangeHUQtyDialog = {
  ...
  expectFieldValue: async ({ field, expectedValue }) =>
    await test.step(`${NAME} - Expect "${field}" to have value "${expectedValue}"`, async () => {
      await ChangeHUQtyDialog.expectVisible();
      await expect(page.getByTestId(field)).toHaveValue(expectedValue);
    }),

  type: async ({ field, value }) =>
    await test.step(`${NAME} - Type into ${field}: ${value}`, async () => {
      await ChangeHUQtyDialog.expectVisible();
