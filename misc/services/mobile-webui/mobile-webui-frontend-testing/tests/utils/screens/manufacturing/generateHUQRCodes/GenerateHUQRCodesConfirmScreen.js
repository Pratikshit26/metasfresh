...
export const GenerateHUQRCodesConfirmScreen = {
  ...
  confirm: async ({ expectNumberOfHUs, expectNumberOfCopies }) =>
    await test.step(`${NAME} - Confirm print`, async () => {
      if (expectNumberOfHUs != null) {
        await expect(page.getByTestId("numberOfHUs-field")).toHaveValue(
          `${expectNumberOfHUs}`,
        );
      }
      if (expectNumberOfCopies != null) {
        await expect(page.getByTestId("numberOfCopies-field")).toHaveValue(
          `${expectNumberOfCopies}`,
        );
      }

      const generatedQRCodesPromise = waitForApiResponse({
        urlContains: "/manufacturing/generateHUQRCodes",
      });
...
const waitForApiResponse = ({ urlContains, timeout = 5000 }) => {
  ...
  const timeoutRef = setTimeout(() => {
    ...
    rejectCallback(
      new Error(`Timeout: No response matched URL part "${urlContains}"`),
    );
  }, timeout);
