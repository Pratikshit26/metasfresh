import { getTab, isBlank, updateTab } from "../../utils";
...
describe("General helpers", () => {
  describe("getTab", () => {
    ...
    it("find tab in a nested array", () => {
      ...
      const tabsData = {
        tabs: [
          ...
          {
            tabId: "3c",
            tabs: [{ tabId: "3c1" }, tab32, { tabId: "3c3" }],
          },
        ],
      };
    ...
    it(`return null, when tab can't be found`, () => {
      const tabsData = {
        tabs: [{ tabId: "1a" }, { tabId: "2b" }, { tabId: "3c" }],
      };
    ...
    it(`return null, when nested tab can't be found`, () => {
      const tabsData = {
        tabs: [
          ...
          {
            tabId: "3c",
            tabs: [{ tabId: "3c1" }, { tabId: "3c2" }, { tabId: "3c3" }],
          },
        ],
      };
  ...
  describe("updateTab", () => {
    it("updates tab in a flat array", () => {
      ...
      expect(updateTab(tabsData, "2b", changes)).toEqual(
        expect.arrayContaining([expect.objectContaining(changes)]),
      );
    });
  ...
  describe("isBlank", () => {
    ...
    it("not blank with spaces", () =>
      expect(isBlank(" aaa   ")).toEqual(false));
  });
});
