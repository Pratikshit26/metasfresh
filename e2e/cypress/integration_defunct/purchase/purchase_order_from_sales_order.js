...
describe("Create Purchase order from sales order", function () {
  ...
  it("Read the fixture", function () {
    cy.fixture("purchase/purchase_order_from_sales_order.json").then((f) => {
      ...
      purchasePriceListVersion = appendHumanReadableNow(
        f["purchasePriceListVersion"],
      );
      salesPriceSystem = appendHumanReadableNow(f["salesPriceSystem"]);
      salesPriceList = appendHumanReadableNow(f["salesPriceList"]);
      salesPriceListVersion = appendHumanReadableNow(
        f["salesPriceListVersion"],
      );
  ...
  it("Create price and product entities", function () {
    ...
    cy.fixture("product/product_price.json").then((productPriceJson) => {
      productPricePM1 = Object.assign(
        new ProductPrice(),
        productPriceJson,
      ).setPriceList(purchasePriceList);
    });
    cy.fixture("product/product_price.json").then((productPriceJson) => {
      productPricePM2 = Object.assign(
        new ProductPrice(),
        productPriceJson,
      ).setPriceList(salesPriceList);
    });
  ...
  it("Create product category, product, vendor and customer entities", function () {
    ...
    let productPrice2;
    cy.fixture("product/product_price.json").then((productPriceJson) => {
      productPrice1 = Object.assign(
        new ProductPrice(),
        productPriceJson,
      ).setPriceList(purchasePriceList);
    });
    cy.fixture("product/product_price.json").then((productPriceJson) => {
      productPrice2 = Object.assign(
        new ProductPrice(),
        productPriceJson,
      ).setPriceList(salesPriceList);
    });
  ...
  it("Create and check the Purchase Order", function () {
    ...
    cy.getStringFieldValue("QtyEnteredTU", true).should(
      "equals",
      soQty.toString(),
    );
    cy.getStringFieldValue("C_BPartner_ID", true).should(
      "contains",
      vendorName,
    );
    cy.pressDoneButton();
