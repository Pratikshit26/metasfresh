describe("Create a Product Category and Products", function () {
  ...
  it("Create a new Product Category and Products", function () {
    ...
    //set AttributeSet
    cy.get(".form-field-M_AttributeSet_ID").find("input").type("T");
    ...
    cy.get(".form-field-M_Product_Category_ID").find("input").type("T");
    cy.get(".input-dropdown-list").should("exist");
    cy.contains(
      ".input-dropdown-list-option",
      "01_TestProductCategory1",
    ).click();

    //set Product Price
    cy.get("tab_M_ProductPrice").click();
    cy.pressAddNewButton();
    cy.get(".form-field-M_PriceList_Version_ID").find("input").type("T");
    cy.get(".input-dropdown-list").should("exist");
    ...
    cy.get(".form-field-field-C_TaxCategory_ID").find("input").type("T");
    cy.get(".input-dropdown-list").should("exist");
    cy.contains(
      ".input-dropdown-list-option",
      "Reduzierter Satz Waren/ DL",
    ).click();
    cy.get(".items-row-2 > .btn").click();

    cy.get(".form-field-...").find(".input-checkbox-tick").click();
  });
});
