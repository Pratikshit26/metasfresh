...
describe("Void Sales Invoice and invoice the billing candidates again", function () {
  ...
  it("Read the fixture", function () {
    cy.fixture(
      "sales/void_a_sales_invoice_then_invoice_the_billing_candidates_again.json",
    ).then((f) => {
      ...
      generateInvoicesNotificationModalText =
        f["generateInvoicesNotificationModalText"];
  ...
  it("Expect only 1 Billing Candidates row and open it", function () {
    ...
    BillingCandidates.getRows().eq(0).dblclick();
  });
  ...
  describe("Ship the Sales Order", function () {
    ...
    it("Expect only 1 Shipment Disposition row and open it", function () {
      ...
      // select the first Table Row and click it (open it)
      // eslint-disable-next-line prettier/prettier
      ShipmentDispositions.getRows().eq(0).dblclick();
    });
    ...
    it("Shipment Disposition checks", function () {
      ...
      cy.getStringFieldValue("C_Order_ID").should(
        "equal",
        salesOrderDocumentNumber,
      );
      cy.getStringFieldValue("QtyOrdered_Calculated").should(
        "equal",
        originalQuantity.toString(10),
      );
      cy.getStringFieldValue("QtyToDeliver ").should(
        "equal",
        originalQuantity.toString(10),
      );
    });
  ...
  describe("Generate Invoice and check it", function () {
    it('Execute action "Generate Invoices"', function () {
      cy.readAllNotifications();
      cy.executeHeaderActionWithDialog(
        "C_Invoice_Candidate_EnqueueSelectionForInvoicing",
      );
  ...
  function checkBillingCandidate(qtyDelivered, qtyInvoiced) {
    ...
    cy.getStringFieldValue("QtyOrdered").should(
      "equal",
      originalQuantity.toString(10),
    );
...
function salesInvoiceChecks(
  ...
  cy.getStringFieldValue("QtyEntered", true).should(
    "be.equal",
    qtyEntered.toString(10),
  );
  cy.pressDoneButton();
