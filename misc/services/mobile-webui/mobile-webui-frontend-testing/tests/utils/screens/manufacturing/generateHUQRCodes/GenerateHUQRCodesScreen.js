...
export const GenerateHUQRCodesScreen = {
  ...
  print: async ({ piTestId, expectNumberOfHUs, expectNumberOfCopies }) =>
    await test.step(`${NAME} - Print ${piTestId}`, async () => {
      await page.getByTestId(piTestId).tap();
      await GenerateHUQRCodesConfirmScreen.waitForScreen();
      return await GenerateHUQRCodesConfirmScreen.confirm({
        expectNumberOfHUs,
