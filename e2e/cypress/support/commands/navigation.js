...
Cypress.Commands.add(
  "openReferencedDocuments",
  (referenceId, retriesLeft = 8) => {
    ...
    if (retriesLeft >= 1) {
      ...
      cy.get(".order-list-panel .order-list-loader", timeout).should(
        "not.exist",
      );
...
Cypress.Commands.add(
  "selectNthRow",
  (rowNumber, modal = false, force = false) => {
...
Cypress.Commands.add(
  ...
  (columnAndValue, modal = false, force = false, single = true) => {
      ...
      cy
        ...
        .should((table) => {
          ...
          columnAndValue.forEach((c) => {
            ...
            expect(
              htmlTable.includes(`data-cy="cell-${c.column}"`),
              `Column ${c.column} exists`,
            ).is.true;
          });
        })
...
Cypress.Commands.add(
  ...
  (columnAndValue, modal = false, force = false) => {
    cy.log(
      `Select HU using BarcodeFilter by ${JSON.stringify(columnAndValue)}`,
    );
    ...
    cy.intercept("POST", new RegExp(RewriteURL.QuickActions)).as(
      quickActionsAlias,
    );
...
Cypress.Commands.add("countAllRows", () => {
  cy.get(".pagination-row .pagination-part .hidden-sm-down").then(
    (totalString) => {
