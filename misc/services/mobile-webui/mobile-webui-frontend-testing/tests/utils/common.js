...
export const FRONTEND_BASE_URL =
  process.env.FRONTEND_BASE_URL || "http://localhost:3001/mobile";
  ...
  try {
    return await Promise.race([
      ...
      ErrorScreen.watchForScreen(async () => {
        throw new Error(
          "Unexpected error screen detected. Usually this is an indicator of development errors. Check console for more info.",
        );
      }),
    ]);
  } finally {
...
export const expectErrorToast = async (title, func) => {
  ...
  return await test.step(`Expect error: ${title} (watcherId=${watcherId})`, async () => {
    const executeFuncFailOnSuccess = async () => {
      await func();
      throw new Error(
        `Expected error toast not detected (watcherId=${watcherId})`,
      );
    };
    ...
    try {
      await Promise.race([
        ...
        ErrorToast.waitToPopup(async (toastLocator) => {
          ...
          const textContent = await toastLocator.textContent();
          console.log(
            `[ OK ] Expected error toast detected (watcherId=${watcherId}): ${textContent}`,
          );
