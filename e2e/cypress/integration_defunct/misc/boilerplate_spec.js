...
describe("New boiler plate (textsnippet) test", function () {
  ...
  it("Create a new boiler plate record", function () {
    ...
    cy.log(
      "There shall be no button, because lang-records are not inserted via UI",
    );
    ...
    cy.writeIntoTextField(
      "TextSnippet",
      `{selectall}{backspace}${trlTextSnippet}`,
      true /*modal*/,
    ); // this fails because we now have two "TextSnippet" fields on the page
