...
let productQty;
let locatorId;
const notificationRegexpExpected = new RegExp(
  `Eigenverbrauch.*wurde erstellt.`,
);
...
it("Read the fixture", function () {
  cy.fixture("inventory/create_disposal_from_handling_unit_editor.json").then(
    (f) => {
...
describe("Create a single HU", function () {
  it("Create Product", function () {
    cy.fixture("product/simple_product.json").then((productJson) => {
      Object.assign(new Product(), productJson).setName(productName).apply();
    });
  });

  it("Create a new single-HU inventory doc", function () {
    cy.fixture("inventory/inventory.json").then((inventoryJson) => {
      const docTypeName = getLanguageSpecific(
        inventoryJson,
        "singleHUInventoryDocTypeName",
      );
...
describe("Open Internal Use from notifications bell and do the checks", function () {
  ...
  it("Check tab Internal Use Line", function () {
    ...
    cy.getStringFieldValue("QtyInternalUse", true).should(
      "equal",
      productQty.toString(10),
    );
