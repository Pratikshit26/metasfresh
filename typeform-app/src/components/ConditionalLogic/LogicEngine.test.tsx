...
class LogicEngine {
  evaluateCondition(condition: LogicCondition, responses: FormResponses): boolean {
  ...
  evaluateConditions(logic: ConditionalLogic, responses: FormResponses): boolean {
  ...
  applyLogic(logic: ConditionalLogic, responses: FormResponses, context: FormContext): void {
  ...
  describe('Complete Logic Flow', () => {
    it('should apply logic when conditions are met', () => {
      const logic: ConditionalLogic = {
        ...
        actions: [
          { type: 'show', target: 'experienceYears' }
        ]
      }
