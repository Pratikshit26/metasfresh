...
export const DistributionLineScreen = {
  ...
  scanHUToMove: async ({ huQRCode, qtyToMove, expectedQtyToMove }) =>
    await test.step(`${NAME} - Scan QR Code`, async () => {
  ...
  clickStepButton: async ({ index }) =>
    await test.step(`${NAME} - Click step ${index}`, async () => {
      await DistributionLineScreen.expectVisible();
      await page
        .getByTestId(`step-${index}-button`)
        .tap({ timeout: FAST_ACTION_TIMEOUT });
      await DistributionStepScreen.waitForScreen();
    }),

  expectNoStepButton: async () =>
    await test.step(`${NAME} - Expect no step buttons`, async () => {
      await DistributionLineScreen.expectVisible();
      await expect(page.getByTestId("step-0-button")).toHaveCount(0);
    }),

  goBack: async () =>
    await test.step(`${NAME} - Go back`, async () => {
      await DistributionLineScreen.expectVisible();
