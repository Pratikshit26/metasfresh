...
describe("New sales order test", function () {
  ...
  describe("create an invoice", function () {
    it("Zoom to the sales order's invoice candidate", function () {
      ...
      cy.intercept(
        "GET",
        `/rest/api/documentView/${invoiceCandidates.windowId}/*?firstRow=0&pageLength=*`,
      ).as(getDataAlias);
    ...
    it("Select all invoice candidates and invoice them", function () {
      ...
      cy.intercept(
        "GET",
        `/rest/api/documentView/${invoiceCandidates.windowId}/*/quickActions?selectedIds=all`,
      ).as(quickActionsAlias);
    ...
    it("Zoom to the new invoice", function () {
      ...
      cy.intercept(
        "GET",
        `/rest/api/window/${invoiceCandidates.windowId}/*/AD_Tab*`,
      ).as(invoiceCandidateDetail);
      ...
      cy.intercept(
        "GET",
        `/rest/api/documentView/${salesInvoices.windowId}/*?firstRow=0&pageLength=*`,
      ).as(getDataAlias);
