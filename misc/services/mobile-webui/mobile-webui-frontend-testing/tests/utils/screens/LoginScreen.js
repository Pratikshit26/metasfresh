...
export const LoginScreen = {
  ...
  login: async ({ username, password, expectDefaultAuthMethod }) =>
    await test.step(`${NAME} - Login with user ${username}`, async () => {
      let authToken = null;
      let language = null;

      page.on("response", async (response) => {
        if (response.url().includes("/api/v2/auth")) {
          // Match the specific API
          const jsonResponse = await response.json();
          authToken = jsonResponse.token;
          language = jsonResponse.language;
          console.log(
            `Intercepted authentication token: ${authToken} from login response`,
            jsonResponse,
          );
        }
      });
