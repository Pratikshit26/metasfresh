...
import {
  BillOfMaterial,
  BillOfMaterialLine,
} from "../../support/utils/billOfMaterial";
...
describe("Create test data", function () {
  it("Read the fixture", function () {
    cy.fixture(
      "manufacturing/manufacturing_with_issuing_from_multiple_CUs.json",
    ).then((f) => {
      // testdata
      categoryName1 = appendHumanReadableNow(f["categoryName1"]);
      categoryName2 = appendHumanReadableNow(f["categoryName2"]);
      productComponentName1 = appendHumanReadableNow(
        f["productComponentName1"],
      );
      productComponentName2 = appendHumanReadableNow(
        f["productComponentName2"],
      );
      productComponentName3 = appendHumanReadableNow(
        f["productComponentName3"],
      );
  ...
  it("Create product components", function () {
    // first component
    cy.fixture("product/simple_productCategory.json").then(
      (productCategoryJson) => {
  ...
  it("Create a BOM for the product", function () {
    cy.fixture("product/bill_of_material.json").then((billMaterialJson) => {
      Object.assign(new BillOfMaterial(), billMaterialJson)
        ...
        .addLine(
          new BillOfMaterialLine()
            .setProduct(productComponentName1)
            .setQuantity(productComponentQty1)
            .setIssueMethod(bomIssueMethod),
        )
        // eslint-disable-next-line prettier/prettier
        .addLine(
          new BillOfMaterialLine()
            .setProduct(productComponentName2)
            .setQuantity(productComponentQty2)
            .setIssueMethod(bomIssueMethod),
        )
        // eslint-disable-next-line prettier/prettier
        .addLine(
          new BillOfMaterialLine()
            .setProduct(productComponentName3)
            .setQuantity(productComponentQty3)
            .setIssueMethod(bomIssueMethod),
        )
        .apply();
    });
  ...
  it("Create 3 HUs", function () {
    Builder.createHUWithStock(productComponentName1, huQty, locatorId).then(
      (huVal) => (huValue1 = huVal),
    );
    Builder.createHUWithStock(productComponentName2, huQty, locatorId).then(
      (huVal) => (huValue2 = huVal),
    );
    Builder.createHUWithStock(productComponentName3, huQty, locatorId).then(
      (huVal) => (huValue3 = huVal),
    );
  });
});
...
describe("Case 1: Process the manufacturing order **after** the components are issued", function () {
  it("Create Manufacturing Order Doc", function () {
    ...
    cy.getStringFieldValue("PP_Product_BOM_ID").should(
      "contain",
      finishedGoodName,
    );
    ...
    cy.getStringFieldValue("C_DocTypeTarget_ID").should(
      "contain",
      expectedManufacturingTargetDocType,
    );
    cy.getStringFieldValue("PriorityRule").should(
      "contain",
      expectedManufacturingPriorityRule,
    );
    cy.getStringFieldValue("QtyEntered").should(
      "contain",
      manufacturingQtyEntered,
    );
    cy.getStringFieldValue("C_UOM_ID").should("contain", eachUOM);

    cy.writeIntoStringField(
      "DateOrdered",
      manufacturingDateOrdered,
      false,
      null,
      true,
    );
    ...
    cy.selectRowByColumnAndValue({
      column: manufacturingOrderComponentsProductColumn,
      value: productComponentName1,
    });
    cy.selectRowByColumnAndValue({
      column: manufacturingOrderComponentsProductColumn,
      value: productComponentName2,
    });
    cy.selectRowByColumnAndValue({
      column: manufacturingOrderComponentsProductColumn,
      value: productComponentName3,
    });
  ...
  it('Run action "Issue/Receipt" and expect 4 rows', function () {
    ...
    cy.selectRowByColumnAndValue(
      createColumnAndValue(
        productComponentName1,
        undefined,
        "CO",
        productComponentQty1,
      ),
      true,
    );
    cy.selectRowByColumnAndValue(
      createColumnAndValue(
        productComponentName2,
        undefined,
        "CO",
        productComponentQty2,
      ),
      true,
    );
    cy.selectRowByColumnAndValue(
      createColumnAndValue(
        productComponentName3,
        undefined,
        "CO",
        productComponentQty3,
      ),
      true,
    );
  });
  ...
  it("Receive the finished good", function () {
    cy.selectRowByColumnAndValue(
      { column: productColumn, value: finishedGoodName },
      true,
    );
    cy.executeQuickAction("WEBUI_PP_Order_Receipt", true);
    cy.getStringFieldValue("M_HU_PI_Item_Product_ID").should(
      "contain",
      expectNoPackingItem,
    );
  ...
  it('Run action "Issue CUs from source HUs" when only component 1 is selected', function () {
    cy.selectRowByColumnAndValue(
      createColumnAndValue(productComponentName1, null, "CO"),
      true,
    );
    cy.executeQuickAction("WEBUI_PP_Order_M_Source_HU_IssueCUQty", true);
    cy.getStringFieldValue("QtyCU").should(
      "equals",
      productComponentQty1.toString(10),
    );
    cy.pressStartButton();
    cy.selectRowByColumnAndValue(
      createColumnAndValue(
  ...
  it('Run action "Issue CUs from source HUs" when both component 2 and 3 are selected', function () {
    ...
    cy.selectRowByColumnAndValue(
      createColumnAndValue(productComponentName2, null, "CO"),
      true,
    )
      .get("body")
      .type("{control}", { release: false })
      .selectRowByColumnAndValue(
        createColumnAndValue(productComponentName3, null, "CO"),
        true,
      )
      .get("body")
      .type("{control}");
  ...
  it("Process the finished good", function () {
    cy.selectRowByColumnAndValue(
      createColumnAndValue(finishedGoodName, undefined, "MP"),
      true,
    );
    cy.executeQuickAction(
      "WEBUI_PP_Order_ChangePlanningStatus_Complete",
      true,
      false,
    );
...
describe("Case 2: Process the manufacturing order **before** the components are issued", function () {
  ...
  it('Run action "Issue CUs from source HUs" when only component 2 is selected', function () {
    ...
    cy.getStringFieldValue("QtyCU").should(
      "equals",
      productComponentQty2.toString(10),
    );
    cy.pressStartButton();
    cy.selectRowByColumnAndValue(
      createColumnAndValue(
  ...
  it("Go to Handling Unit Editor and check expected qty of finished good", function () {
    ...
    cy.selectRowByColumnAndValue(
      { column: qtyCuColumn, value: manufacturingQtyEntered },
      false,
      false,
      false,
...
function selectHuAsSource(productValue, huValue) {
  cy.selectRowByColumnAndValue(
    { column: productColumn, value: productValue },
    true,
  );
  ...
  cy.selectRightTable().within(() => {
    cy.selectRowByColumnAndValue(
      { column: huSelectionCodeColumn, value: huValue },
      false,
      true,
    );
  });
  cy.executeQuickAction(
    "WEBUI_PP_Order_HUEditor_Create_M_Source_HUs",
    true,
    false,
  );
