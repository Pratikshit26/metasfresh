...
export const PickingJobLineScreen = {
  ...
  clickManufactureButton: async () =>
    await test.step(`${NAME} - Click Manufacture button`, async () => {
      await page.getByTestId(`PickFromManufacturingOrder-button`).tap();
      await ManufacturingJobScreen.waitForScreen();
    }),

  clickPickHUButton: async () =>
    await test.step(`${NAME} - Click Pick HU button`, async () => {
      await page.getByTestId(`PickHU-button`).tap();
      await GetQuantityDialog.waitForDialog();
    }),

  clickStepButton: async ({ index }) =>
    await test.step(`${NAME} - Click step ${index} button`, async () => {
