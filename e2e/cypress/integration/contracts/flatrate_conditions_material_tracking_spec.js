...
import {
  ...
  ContractConditions,
  ProductAllocation,
} from "../../support/utils/contract_conditions";
import {
  LagerKonferenz,
  LagerKonferenzVersion,
} from "../../support/utils/contract_lagerKonferenz";

import {
  Product,
  ProductCategory,
  ProductPrice,
} from "../../support/utils/product";
...
import {
  StorageConferenceVersion,
  CostLine,
  LagerKonfMonth,
  AdditionalContribution,
} from "../../support/utils/storage_conferenceversion";
...
it("Read the fixture", function () {
  cy.fixture("contracts/flatrate_conditions_material_tracking_spec.json").then(
    (f) => {
      ...
      processingFeeProductName = appendHumanReadableNow(
        f["processingFeeProductName"],
      );
      productionProductName = appendHumanReadableNow(
        f["productionProductName"],
      );
      witholdingProductName = appendHumanReadableNow(
        f["witholdingProductName"],
      );

      rawProductName = appendHumanReadableNow(f["rawProductName"]);

      calendarName = appendHumanReadableNow(f["calendarName"]);
      contactTransitionName = appendHumanReadableNow(
        f["contactTransitionName"],
      );
      serviceProductType = f["serviceProductType"];
      itemProductType = f["itemProductType"];
      contractConditionsName = appendHumanReadableNow(
        f["contractConditionsName"],
      );
...
it("Create transition", function () {
  new ContractTransition({ name: contactTransitionName })
...
it("Create products for additional contribution tab", function () {
  createProduct(
    contributionProduct1,
    itemProductType,
    productCategoryName,
    productPrice,
  );
  createProduct(
    contributionProduct2,
    itemProductType,
    productCategoryName,
    productPrice,
  );
...
it("Create new Storage conference version", function () {
  cy.fixture("contract/storage_conferenceversion.json").then(
    (conferenceVersionJson) => {
