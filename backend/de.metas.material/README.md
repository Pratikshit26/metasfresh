...
- client
  - offers a richer interface (e.g. with UOM) to callers. Supposed to be _external_ to the standalone service.
  - depends on commons
...
The material disposition framework's data mostly consists of _candidate_ records (table name `MD_Candidate`).
They are created by the system in an _incremental_ fashion, accordingly to the system's master data (BOMs, Distribution networks etc) and can be inspected and changed by users.
...
A stock candidate can be the _child_ of a demand candidate or the _parent_ of a supply.
...
It _decreases_ the quantity of its child's `stock` record. That's because the demand candidate is about compensating for a planned removal of goods.
...
It _increases_ the quantity of its parent `stock` record
