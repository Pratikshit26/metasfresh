...
import {
  PurchaseOrder,
  PurchaseOrderLine,
} from "../../support/utils/purchase_order";
import {
  applyFilters,
...
describe("Partial material withdrawal in handling unit editor with Materialentnahmelager", function () {
  ...
  it("Read the fixture", function () {
    cy.fixture("logistics/materialentnahme_partial_HU.json").then((f) => {
      productForPackingMaterialTU = appendHumanReadableNow(
        f["productForPackingMaterialTU"],
      );
      productForPackingMaterialLU = appendHumanReadableNow(
        f["productForPackingMaterialLU"],
      );
      ...
      packingInstructionsNameForTU = appendHumanReadableNow(
        f["packingInstructionsNameForTU"],
      );
      packingInstructionsNameForLU = appendHumanReadableNow(
        f["packingInstructionsNameForLU"],
      );
      ...
      packingInstructionsVersionForTU = appendHumanReadableNow(
        f["packingInstructionsVersionForTU"],
      );
      packingInstructionsVersionForLU = appendHumanReadableNow(
        f["packingInstructionsVersionForLU"],
      );
    });
  });
  ...
  it("create packing instruction for handling unit - Transport unit", function () {
    cy.fixture("product/packing_instructions.json").then(
      (packingInstructionsJson) => {
  ...
  describe("Create a purchase order and Material Receipts", function () {
    it("Create a purchase order and visit Material Receipt Candidates", function () {
      new PurchaseOrder()
        ...
        .setPoReference("test")
        .addLine(
          new PurchaseOrderLine().setProduct(productName1).setQuantity(5),
        )
        .apply();
      cy.completeDocument();
    });
    ...
    it("Select first row - related to LU quantity and extract 1 from there", function () {
      ...
      cy.selectNthRow(0).find(".Quantity").should("contain", "40");
    });
  });
});
