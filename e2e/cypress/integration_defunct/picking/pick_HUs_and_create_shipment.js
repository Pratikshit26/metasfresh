...
describe("Create test data", function () {
  ...
  it("Create first single-HU inventory doc", function () {
    cy.fixture("picking/pick_HUs_and_create_shipment.json").then((f) => {
      Builder.createHUWithStock(
        f["productName"],
        f["productQty"],
        f["locatorId"],
      ).then((huVal) => (huValue1 = huVal));
    });
  });
  ...
  it("Create Sales Order", function () {
    cy.fixture("picking/pick_HUs_and_create_shipment.json").then((f) => {
      new SalesOrder()
        ...
        .addLine(
          new SalesOrderLine()
            .setProduct(f["productName"])
            .setQuantity(f["soProductQuantity"]),
        )
        .apply();
      cy.completeDocument();

      cy.getCurrentWindowRecordId().then((id) => (soRecordId = id));
      cy.getStringFieldValue("DocumentNo").then(
        (docNO) => (soDocNumber = docNO),
      );
    });
  });
});
