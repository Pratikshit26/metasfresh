...
test("Assemble/Manufacture while picking test", async ({ page }) => {
  ...
  await PickingJobScreen.setTargetLU({
    lu: masterdata.packingInstructions.P2x25x5.luName,
  });
  ...
  const generatedQRCode = await ManufacturingJobScreen.generateSingleHUQRCode({
    piTestId: masterdata.packingInstructions.P2x25x5.tuPITestId,
  });
  ...
  await MaterialReceiptLineScreen.selectExistingHUTarget({
    huQRCode: generatedQRCode,
  });
