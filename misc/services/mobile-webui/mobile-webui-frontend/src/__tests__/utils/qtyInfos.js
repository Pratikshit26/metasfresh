...
describe('qtyInfos tests', () => {
    ...
    describe('toNumber', () => {
        it('from null', () => {
            expect(qtyInfos.toNumber(null)).toEqual(null);
        });
        it('from object: qtyStr missing', () => {
            expect(qtyInfos.toNumber({ qty: 123.456 })).toEqual(123.456);
        });
        it('from object: number with trailing zeroes', () => {
            expect(qtyInfos.toNumber({ qty: 1, qtyStr: '1.0000' })).toEqual(1);
        });
        it('from string', () => {
            expect(qtyInfos.toNumber('456.7890')).toEqual(456.789);
        });
    });
});
