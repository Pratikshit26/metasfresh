...
class AnalyticsService {
  ...
  calculateMetrics(formId: string) {
    const formEvents = this.events.filter(e => e.properties.formId === formId)
    
    const views = formEvents.filter(e => e.event === 'form_view').length
    const starts = formEvents.filter(e => e.event === 'form_start').length
    const completions = formEvents.filter(e => e.event === 'form_complete').length
    const abandons = formEvents.filter(e => e.event === 'form_abandon').length
    
    const completionRate = starts > 0 ? (completions / starts) * 100 : 0
    
    const completedForms = formEvents.filter(e => e.event === 'form_complete')
    const totalTime = completedForms.reduce((sum, e) => sum + (e.properties.completionTime || 0), 0)
  ...
  getDropOffPoints(formId: string) {
    const questionAnswers = this.events.filter(e => 
      e.event === 'question_answer' && e.properties.formId === formId
    )
    
    const abandons = this.events.filter(e => 
      e.event === 'form_abandon' && e.properties.formId === formId
    )
...
const AnalyticsDashboard = ({ formId, analyticsService }: { formId: string, analyticsService: AnalyticsService }) => {
  ...
  return (
    <div data-testid="analytics-dashboard">
      <div data-testid="metrics">
        ...
        <div data-testid="completions-metric">Completions: {metrics.completions}</div>
        <div data-testid="completion-rate-metric">Completion Rate: {metrics.completionRate}%</div>
        <div data-testid="average-time-metric">Average Time: {metrics.averageTime}s</div>
      </div>
...
describe('Analytics & Metrics', () => {
  ...
  describe('AnalyticsService', () => {
    it('tracks events correctly', () => {
      ...
      analyticsService.track('form_start', { formId: 'test-form', respondentId: 'user1' })
    ...
    it('calculates basic metrics correctly', () => {
      ...
      analyticsService.track('form_complete', { formId, respondentId: 'user1', completionTime: 120 })
    ...
    it('calculates drop-off points', () => {
      ...
      expect(dropOffPoints.find(p => p.questionId === 'q1')?.dropRate).toBe(2)
      expect(dropOffPoints.find(p => p.questionId === 'q2')?.dropRate).toBe(1)
    })
  })
  ...
  describe('AnalyticsDashboard Component', () => {
    it('displays metrics correctly', async () => {
      ...
      render(<AnalyticsDashboard formId={formId} analyticsService={analyticsService} />)

      await waitFor(() => {
        expect(screen.getByTestId('views-metric')).toHaveTextContent('Views: 1')
        expect(screen.getByTestId('starts-metric')).toHaveTextContent('Starts: 1')
        expect(screen.getByTestId('completions-metric')).toHaveTextContent('Completions: 1')
        expect(screen.getByTestId('completion-rate-metric')).toHaveTextContent('Completion Rate: 100%')
        expect(screen.getByTestId('average-time-metric')).toHaveTextContent('Average Time: 180s')
      })
    })

    it('shows drop-off analysis', async () => {
      const formId = 'test-form'
      analyticsService.track('form_abandon', { formId, lastQuestionId: 'question-1' })
      analyticsService.track('form_abandon', { formId, lastQuestionId: 'question-2' })

      render(<AnalyticsDashboard formId={formId} analyticsService={analyticsService} />)

      await waitFor(() => {
        expect(screen.getByText('Drop-off Points')).toBeInTheDocument()
        expect(screen.getByTestId('drop-off-question-1')).toHaveTextContent('Question question-1: 1 abandons')
        expect(screen.getByTestId('drop-off-question-2')).toHaveTextContent('Question question-2: 1 abandons')
      })
    })
  })
  ...
  describe('FormResponseTracker Component', () => {
    ...
    it('tracks form view on mount', () => {
      const formId = 'test-form'
      render(<FormResponseTracker formId={formId} questions={mockQuestions} analyticsService={analyticsService} />)
    ...
    it('shows progress correctly', async () => {
      ...
      expect(screen.getByTestId('progress')).toHaveTextContent('Question 1 of 3')
      
      fireEvent.click(screen.getByTestId('next-button'))
      expect(screen.getByTestId('progress')).toHaveTextContent('Question 2 of 3')
    })
  })
})