...
export const MaterialReceiptLineScreen = {
  ...
  clickReceiveTargetButton: async () =>
    await test.step(`${NAME} - Click receive target button`, async () => {
      await page.getByTestId("receive-target-button").tap();
      await ReceiptReceiveTargetScreen.waitForScreen();
    }),

  selectNewLUTarget: async ({ luPIItemTestId }) =>
    await test.step(`${NAME} - Select New LU target "${luPIItemTestId}"`, async () => {
  ...
  selectNewTUTarget: async ({ tuPIItemProductTestId }) =>
    await test.step(`${NAME} - Select New TU target "${tuPIItemProductTestId}"`, async () => {
      await MaterialReceiptLineScreen.clickReceiveTargetButton();
      await ReceiptReceiveTargetScreen.clickNewHUButton();
  ...
  selectExistingHUTarget: async ({ huQRCode }) =>
    await test.step(`${NAME} - Select existing HU target`, async () => {
      await MaterialReceiptLineScreen.clickReceiveTargetButton();
  ...
  receiveQty: async ({ qtyEntered, expectQtyEntered }) =>
    await test.step(`${NAME} - Receive qty ${qtyEntered ? qtyEntered : ""}`, async () => {
