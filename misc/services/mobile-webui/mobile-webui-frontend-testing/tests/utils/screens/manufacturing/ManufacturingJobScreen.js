...
export const ManufacturingJobScreen = {
  ...
  goBackToPickingJobLine: async () =>
    await test.step(`${NAME} - Go back to picking job line`, async () => {
      await page.locator(ID_BACK_BUTTON).tap();
      await PickingJobLineScreen.waitForScreen();
    }),

  generateSingleHUQRCode: async ({
    piTestId,
    expectNumberOfHUs,
    expectNumberOfCopies,
  }) =>
    await test.step(`${NAME} - Generate HU QR codes`, async () => {
      await page.getByTestId(`generateHUQRCodes-button`).tap();
      await GenerateHUQRCodesScreen.waitForScreen();
      const generatedQRCodesResult = await GenerateHUQRCodesScreen.print({
        piTestId,
        expectNumberOfHUs: 1,
        expectNumberOfCopies: 1,
      });
  ...
  issueRawProduct: async ({ index, qrCode, expectQtyEntered }) =>
    await test.step(`${NAME} - Issue line ${index}`, async () => {
  ...
  clickIssueButton: async ({ index }) =>
    await test.step(`${NAME} - Click issue button ${index}`, async () => {
  ...
  clickReceiveButton: async ({ index }) =>
    await test.step(`${NAME} - Click receive button ${index}`, async () => {
      await ManufacturingJobScreen.expectVisible();
  ...
  complete: async () =>
    await test.step(`${NAME} - Complete`, async () => {
      await ManufacturingJobScreen.expectVisible();
