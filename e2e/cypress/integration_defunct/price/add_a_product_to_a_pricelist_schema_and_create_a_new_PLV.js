...
import {
  PriceListSchema,
  PriceListSchemaLine,
} from "../../support/utils/price_list_schema";
...
it("Read fixture and prepare the names", function () {
  cy.fixture(
    "price/add_a_product_to_a_pricelist_schema_and_create_a_new_PLV.json",
  ).then((f) => {
    priceSystemName = appendHumanReadableNow(f["priceSystemName"]);
    priceListName = appendHumanReadableNow(f["priceListName"]);
    priceListVersionName = appendHumanReadableNow(f["priceListVersionName"]);

    priceListSchemaVersionName = appendHumanReadableNow(
      f["priceListSchemaVersionName"],
    );
    priceListVersion2ValidFrom = f["priceListVersion2ValidFrom"];

    // the magics
    const appendedDate = appendHumanReadableNow("").substr(1);
    priceListVersionNameSearch1 = new RegExp(
      f["priceListVersionNameSearch1_part1"] +
        ".*" +
        appendedDate +
        ".*" +
        f["priceListVersionNameSearch1_part2"],
    );
    priceListVersionNameSearch2 = new RegExp(
      f["priceListVersionNameSearch2_part1"] +
        ".*" +
        appendedDate +
        ".*" +
        f["priceListVersionNameSearch2_part2"],
    );
...
describe("Create Price List Schema for Product", function () {
  it("Create Price List Schema", function () {
    new PriceListSchema()
      ...
      .addLine(
        new PriceListSchemaLine()
          .setProduct(productName1)
...
describe("Create new Price List Version using the Price List Schema", function () {
  ...
  it("Check the new Product Price", function () {
    ...
    cy.getStringFieldValue("PriceStd").should((val) => {
      expect(parseFloat(val)).to.be.closeTo(
        originalPriceStd + surchargeAmount,
        0.01,
      );
    });
    ...
    cy.getStringFieldValue("M_Product_Category_ID").should(
      "contain",
      categoryName,
    );
    cy.getStringFieldValue("M_PriceList_Version_ID").should(
      "contain",
      priceListName,
    );
    cy.getStringFieldValue("C_UOM_ID").should("contain", originalUOM);
    cy.getStringFieldValue("C_TaxCategory_ID").should(
      "contain",
      originalTaxCategory,
    );
  });
