...
import { initialState as tableQuickInputHandlerState } from "../../../reducers/tableQuickInputHandler";
...
describe("TableQuickInput", () => {
  ...
  it("renders without error 22s", (done) => {
    const moduleMock = jest.requireActual(
      "../../../actions/TableQuickInputActions",
    );
    ...
    const wrapper = mount(
      <Provider store={store}>
        <ConnectedTableQuickInput
          {...localProps}
          fetchQuickInputLayout={fetchQuickInputLayoutMock}
        />
      </Provider>,
    );
