...
describe("Create test data", function () {
  it("Read fixture and prepare the names", function () {
    cy.fixture("picking/pick_in_a_new_HU.json").then((f) => {
      ...
      productForPackingMaterial = appendHumanReadableNow(
        f["productForPackingMaterial"],
      );
      packingInstructionsName = appendHumanReadableNow(
        f["packingInstructionsName"],
      );
  ...
  it("Create bPartner", function () {
    cy.fixture("sales/simple_customer.json").then((customerJson) => {
      new BPartner({ ...customerJson, name: bPartnerName })
...
describe("Pick the SO", function () {
  ...
  it("Mark the HU as source", function () {
    ...
    cy.selectRightTable().within(() => {
      cy.selectRowByColumnAndValue(
        { column: huSelectionHuCodeColumn, value: huValue },
        false,
        true,
      );
    });
    cy.executeQuickAction(
      "WEBUI_Picking_HUEditor_Create_M_Source_HUs",
      true,
      false,
    );
    cy.selectRightTable().within(() => {
      ...
      cy.selectRowByColumnAndValue(
        { column: pickingHuCodeColumn, value: huValue },
        false,
        true,
      );
    });
