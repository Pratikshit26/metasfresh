...
test("Move HU", async ({ page }) => {
  ...
  await HUManagerScreen.expectValue({
    name: "qty-value",
    expectedValue: "80 PCE",
  });
...
test("Change Qty", async ({ page }) => {
  ...
  await HUManagerScreen.changeQty({
    expectQtyEntered: "80",
    qtyEntered: "100",
    description: "test",
  });
...
test("Change Clearance Status", async ({ page }) => {
  ...
  await HUManagerScreen.changeClearanceStatus({
    status: CLEARANCE_STATUS_Quarantined,
    note: "my status note",
  });
  await HUManagerScreen.expectValue({
    name: "clearanceStatus-value",
    expectedInternalValue: CLEARANCE_STATUS_Quarantined,
  });
  await HUManagerScreen.expectValue({
    name: "clearanceNote-value",
    expectedValue: "my status note",
  });
});
