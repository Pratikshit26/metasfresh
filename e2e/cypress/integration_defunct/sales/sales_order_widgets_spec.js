describe("Sales order window widgets test", function () {
  ...
  context("Add/remove product", function () {
    it("Add product using batch entry", function () {
      cy.intercept("POST", "/rest/api/window/143/1000489/187/quickInput").as(
        "postAddress",
      );
      ...
      cy.wait("@postAddress").then((xhr) => {
        ...
        cy.intercept(
          "PATCH",
          `/rest/api/window/143/1000489/187/quickInput/${requestId}`,
        ).as("patchAddress");
        ...
        cy.get("#lookup_M_HU_PI_Item_Product_ID input").should(
          "have.value",
          "IFCO 6410 x 10 Stk",
        );
  ...
  context("Edit product in list", function () {
    it("Change product attributes", function () {
      cy.get(".ProductAttributes")
        .find(".productattributes-cell")
        .last()
        .dblclick();
