...
export const PickingJobScreen = {
  waitForScreen: async () =>
    await step(`${NAME} - Wait for screen`, async () => {
      await containerElement().waitFor({ timeout: SLOW_ACTION_TIMEOUT });
    }),

  scanPickFromHU: async ({ qrCode }) =>
    await step(`${NAME} - Scan pick from HU ${qrCode}`, async () => {
      const button = page.getByTestId(
        `scan-activity-${ACTIVITY_ID_ScanPickFromHU}-button`,
      );
      ...
      await button
        .locator(".indicator-green")
        .waitFor({ state: "attached", timeout: FAST_ACTION_TIMEOUT });
    }),

  scanPickingSlot: async ({ qrCode }) =>
    await step(`${NAME} - Scan picking slot ${qrCode}`, async () => {
      const button = page.locator(
        `#scan-activity-${ACTIVITY_ID_ScanPickingSlot}-button`,
      );
      await button.waitFor();
      await expect(button).toBeEnabled();
      await button.tap();
  ...
  clickLUTargetButton: async () =>
    await step(`${NAME} - Click LU target button`, async () => {
      await page.getByTestId("targetLU-button").tap();
    }),
  setTargetLU: async ({ lu }) =>
    await step(`${NAME} - Set target LU to ${lu}`, async () => {
  ...
  closeTargetLU: async () =>
    await step(`${NAME} - Close target LU`, async () => {
      await PickingJobScreen.clickLUTargetButton();
      await SelectPickTargetLUScreen.clickCloseTargetButton();
      await PickingJobScreen.waitForScreen();
    }),

  clickTUTargetButton: async () =>
    await step(`${NAME} - Click TU target button`, async () => {
      await page.getByTestId("targetTU-button").tap();
    }),
  setTargetTU: async ({ tu }) =>
    await step(`${NAME} - Set target TU to ${tu}`, async () => {
  ...
  closeTargetTU: async () =>
    await step(`${NAME} - Close target TU`, async () => {
      await PickingJobScreen.clickTUTargetButton();
      await SelectPickTargetTUScreen.clickCloseTargetButton();
      await PickingJobScreen.waitForScreen();
    }),

  pickHU: async ({ qrCode, expectQtyEntered, catchWeightQRCode }) =>
  ...
  clickLineButton: async ({ index }) =>
    await step(`${NAME} - Click line ${index}`, async () => {
      await page.locator(`#line-0-${index - 1}-button`).tap();
      //await PickingJobLineScreen.waitForScreen();
    }),

  abort: async () =>
    await step(`${NAME} - Abort`, async () => {
  ...
  complete: async () =>
    await step(`${NAME} - Complete`, async () => {
