...
export const HUManagerScreen = {
  ...
  expectVisible: async () =>
    await test.step(`${NAME} - Expect screen to be displayed`, async () => {
      await expect(containerElement()).toBeVisible();
    }),

  waitForHUInfoPanel: async () =>
    await test.step(`${NAME} - Wait for HU Info Panel`, async () => {
      await page
        .getByTestId("huinfo-table")
        .waitFor({ timeout: SLOW_ACTION_TIMEOUT });
    }),

  scanHUQRCode: async ({ huQRCode }) =>
    await test.step(`${NAME} - Scan HU QR Code`, async () => {
  ...
  expectValue: async ({
    ...
    return await test.step(`${NAME} - Expect "${name}" to be "${stepValue}"`, async () => {
      if (expectedValue !== undefined)
        await expect(page.getByTestId(name)).toHaveText(expectedValue);
      if (expectedInternalValue !== undefined)
        await expect(page.getByTestId(name)).toHaveAttribute(
          "data-internalvalue",
          expectedInternalValue,
        );
  ...
  expectValueMissing: async ({ name }) =>
    HUManagerScreen.expectValue({ name, missing: true }),

  clickButton: async ({ testId }) =>
    await test.step(`${NAME} - Click "${testId}" button`, async () => {
      await page.getByTestId(testId).tap();
    }),

  move: async ({ qrCode }) =>
    await test.step(`${NAME} - Move HU`, async () => {
      await HUManagerScreen.clickButton({ testId: "move-button" });
      await HUMoveScreen.waitForScreen();
      await HUMoveScreen.move({ qrCode });
    }),

  changeQty: async ({ expectQtyEntered, qtyEntered, description }) =>
    await test.step(`${NAME} - Change Qty`, async () => {
      await HUManagerScreen.clickButton({ testId: "change-qty-button" });
      await ChangeHUQtyDialog.waitForDialog();
      if (expectQtyEntered != null)
        await ChangeHUQtyDialog.expectFieldValue({
          field: "qtyEntered",
          expectedValue: expectQtyEntered,
        });
      if (qtyEntered != null)
        await ChangeHUQtyDialog.type({
          field: "qtyEntered",
          value: qtyEntered,
        });
      if (description != null)
  ...
  changeClearanceStatus: async ({ status, note }) =>
    await test.step(`${NAME} - Change Clearance Status`, async () => {
