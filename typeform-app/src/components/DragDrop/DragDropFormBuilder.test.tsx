...
const QuestionTypesPalette = ({ onAddQuestion }: { onAddQuestion: (type: string) => void }) => {
  const questionTypes = [
      ...
      <div className="question-content">
        {isEditing ? (
          <div data-testid={`edit-mode-${question.id}`}>
            ...
            <button onClick={() => setIsEditing(false)} data-testid={`cancel-${question.id}`}>
              Cancel
            </button>
          </div>
        ) : (
        ...
        <div data-testid="form-preview" className="form-preview">
          ...
          <div data-testid="preview-questions">
            {questions.map(question => (
              <div key={`preview-${question.id}`} data-testid={`preview-${question.id}`}>
...
describe('Drag & Drop Form Builder', () => {
  ...
  describe('DraggableQuestion', () => {
    ...
    it('renders question in view mode', () => {
      render(<DraggableQuestion question={mockQuestion} onUpdate={mockOnUpdate} onDelete={mockOnDelete} />)

      expect(screen.getByTestId('view-mode-test-question')).toBeInTheDocument()
      expect(screen.getByTestId('question-title-test-question')).toHaveTextContent('Test Question')
    ...
    it('switches to edit mode', () => {
      ...
      expect(screen.getByTestId('title-input-test-question')).toHaveValue('Test Question')
    })
  ...
  describe('DragDropFormBuilder', () => {
    it('renders empty canvas initially', () => {
      ...
      expect(screen.getByText('Drag question types here to build your form')).toBeInTheDocument()
    })
    ...
    it('updates question titles inline', async () => {
      ...
      await waitFor(() => {
        ...
        fireEvent.change(titleInput, { target: { value: 'Custom Question Title' } })
