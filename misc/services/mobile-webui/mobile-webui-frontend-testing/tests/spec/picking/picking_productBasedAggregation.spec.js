    ...
    request: {
      ...
      handlingUnits: {
        P1_HU: {
          product: "P1",
          warehouse: "wh",
          packingInstructions: "P1_20x4",
        },
        P2_HU: {
          product: "P2",
          warehouse: "wh",
          packingInstructions: "P2_7x3",
        },
        P2_HU_2: {
          product: "P2",
          warehouse: "wh",
...
test("Product based aggregation", async ({ page }) => {
  ...
  await test.step("Picking job for P1", async () => {
    ...
    await PickingJobScreen.scanPickFromHU({
      qrCode: masterdata.handlingUnits.P1_HU.qrCode,
    });
    await PickingJobScreen.scanPickingSlot({
      qrCode: masterdata.pickingSlots.slot1.qrCode,
    });
    await PickingJobScreen.setTargetLU({
      lu: masterdata.packingInstructions.P1_20x4.luName,
    });
    ...
    await test.step("Line 2 - Pick entirely", async () => {
      await PickingJobScreen.scanPickingSlot({
        qrCode: masterdata.pickingSlots.slot2.qrCode,
      });
    ...
    await test.step("Line 3 - Pick entirely", async () => {
      await PickingJobScreen.scanPickingSlot({
        qrCode: masterdata.pickingSlots.slot3.qrCode,
      });
  ...
  await test.step("Picking job for P2", async () => {
    ...
    await PickingJobScreen.scanPickFromHU({
      qrCode: masterdata.handlingUnits.P2_HU.qrCode,
    });
    await PickingJobScreen.scanPickingSlot({
      qrCode: masterdata.pickingSlots.slot1.qrCode,
    });
    await PickingJobScreen.setTargetLU({
      lu: masterdata.packingInstructions.P2_7x3.luName,
    });
    ...
    await test.step("Line 2 - First scan a new pick from HU and then pick entirely again", async () => {
      await PickingJobScreen.scanPickFromHU({
        qrCode: masterdata.handlingUnits.P2_HU_2.qrCode,
      });
      await PickingJobScreen.clickLineButton({ index: 2 });
      await GetQuantityDialog.fillAndPressDone({ expectQtyEntered: 6 /*TU*/ });
      await PickingJobScreen.waitForScreen();
    ...
    await test.step("Line 3 - Scan a new pick from HU and then pick entirely again", async () => {
      await PickingJobScreen.scanPickFromHU({
        qrCode: masterdata.handlingUnits.P2_HU_3.qrCode,
      });
      await PickingJobScreen.clickLineButton({ index: 3 });
