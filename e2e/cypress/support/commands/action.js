...
Cypress.Commands.add("executeHeaderActionWithDialog", (actionName) => {
  describe("Fire header action with a certain name and expect a modal dialog to pop up within 10 secs", function () {
    cy.intercept("GET", new RegExp(RewriteURL.DocumentLayout)).as(
      "dialogLayout",
    );
...
Cypress.Commands.add(
  "executeQuickAction",
  (actionName, modal = false, isDialogExpected = true) => {
...
Cypress.Commands.add(
  "executeQuickActionWithRightSideTable",
  (actionName, isModal = false) => {
