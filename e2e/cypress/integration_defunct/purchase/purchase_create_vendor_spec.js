...
describe("purchase - vendor spec", function () {
  ...
  it("Create test data", function () {
    new DiscountSchema(discountSchemaName)
      .setValidFrom(discountSchemaValidFrom)
      .apply();

    cy.fixture("purchase/simple_vendor.json").then((vendorJson) => {
      new BPartner({ name: vendorName, ...vendorJson })
  ...
  it("Create a vendor with two contacts", function () {
    ...
    cy.location().then((location) => {
      ...
      cy.request(apiUrl).then((response) => {
        ...
        expect(
          bpartnerJson[0].fieldsByName.CompanyName.value,
          "bpartnerJson - CompanyName",
        ).to.eq(vendorName);
        expect(
          bpartnerJson[0].fieldsByName.Name.value,
          "bpartnerJson - Name",
        ).to.eq(vendorName);
        expect(
          bpartnerJson[0].fieldsByName.Name2.value,
          "bpartnerJson - Name2",
        ).to.eq(vendorName);

        const expectedDocumentSummary = `${vendorName} ${bpartnerJson[0].fieldsByName.Value.value}`;
        expect(
          bpartnerJson[0].fieldsByName.V$DocumentSummary.value,
          "V$DocumentSummary",
        ).to.eq(expectedDocumentSummary);
        ...
        cy.wrap(bpartnerJson).toMatchSnapshot(
          `purchase_create_vendor_bpartner - apiUrl=${apiUrl}`,
        );
