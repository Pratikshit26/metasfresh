...
test("Simple distribution test", async ({ page }) => {
  const {
    login,
    warehouseFromFacetId,
    launcherTestId,
    huQRCode,
    dropToLocatorQRCode,
  } = await createMasterdata({ qtyToMove: 100 });
...
test("Distribution using 2 steps to pick the needed qty.", async ({ page }) => {
  const {
    ...
    huQRCode,
    dropToLocatorQRCode,
  } = await createMasterdata({ qtyToMove: 80 });
...
test("Pick & Unpick in distribution step screen", async ({ page }) => {
  const { login, warehouseFromFacetId, launcherTestId, huQRCode } =
    await createMasterdata({ qtyToMove: 100 });
