...
describe("Reverse Empties Return", function () {
  ...
  describe("Create Packing Materials", function () {
    ...
    it("Add Products to Packing Materials", function () {
      ...
      new PackingMaterial()
        ...
        .setProduct(productName2)
        .apply();
      // eslint-disable-next-line
      new PackingMaterial()
        .setName(productName3)
        .setProduct(productName3)
        .apply();
    });
  });
  ...
  describe("Reverse the Empties Return and check", function () {
    ...
    it("Check document data", function () {
      ...
      cy.getTextFieldValue("Description").should(
        "contain",
        originalDocumentNumber,
      );

      cy.openAdvancedEdit();
      cy.getStringFieldValue("Reversal_ID", true).should(
        "contains",
        originalDocumentNumber,
      );
      cy.pressDoneButton();
    ...
    it("Check document line data", function () {
      productList.forEach((productName, index) => {
        ...
        cy.getStringFieldValue("MovementQty", true).should(
          "equals",
          lineValue.toString(10),
        );
  ...
  function createEmptiesReturn() {
    it('Open Material Receipt Candidates and execute action "Empties Return"', function () {
      ...
      cy.executeHeaderAction(
        "WEBUI_M_ReceiptSchedule_CreateEmptiesReturnsToVendor",
      );
    });
