...
const validateField = (question: any, value: any) => {
  ...
  if (question.type === 'text' && question.minLength && value && value.length < question.minLength) {
    return `Minimum ${question.minLength} characters required`
  }
  
  if (question.type === 'text' && question.maxLength && value && value.length > question.maxLength) {
...
const FormValidator = ({ questions, onValidate }: { questions: any[], onValidate: (errors: any, isValid: boolean) => void }) => {
...
describe('Form Validation', () => {
  ...
  describe('Field Validation', () => {
    ...
    it('validates email format', () => {
      const question = { id: '1', type: 'email', title: 'Email', required: true }
      
      expect(validateField(question, 'invalid-email')).toBe('Please enter a valid email address')
      expect(validateField(question, 'valid@email.com')).toBeNull()
    })

    it('validates minimum length', () => {
      const question = { id: '1', type: 'text', title: 'Password', required: true, minLength: 8 }
      
      expect(validateField(question, 'short')).toBe('Minimum 8 characters required')
      expect(validateField(question, 'longenough')).toBeNull()
    })

    it('validates maximum length', () => {
      const question = { id: '1', type: 'text', title: 'Bio', required: false, maxLength: 100 }
      const longText = 'a'.repeat(101)
      
      expect(validateField(question, longText)).toBe('Maximum 100 characters allowed')
      expect(validateField(question, 'short text')).toBeNull()
    })

    it('allows empty optional fields', () => {
      const question = { id: '1', type: 'text', title: 'Optional', required: false }
  ...
  describe('FormValidator Component', () => {
    ...
    it('renders form fields', () => {
      render(<FormValidator questions={questions} onValidate={mockOnValidate} />)
    ...
    it('shows validation errors', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('error-1')).toHaveTextContent('This field is required')
        expect(screen.getByTestId('error-2')).toHaveTextContent('This field is required')
      })
    })
    ...
    it('validates email format in real-time', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('error-2')).toHaveTextContent('Please enter a valid email address')
      })
    })
