...
it("Read the fixture", function () {
  cy.fixture(
    "materialDisposition/create_material_dispo_masterdata_and_sales_order_spec.json",
  ).then((f) => {
    const dateOverride = null;
    mainProductName = appendHumanReadableNow(
      f["mainProductName"],
      dateOverride,
    );
    productCategoryName = appendHumanReadableNow(
      f["productCategoryName"],
      dateOverride,
    );
    customerName = appendHumanReadableNow(f["customerName"], dateOverride);

    priceSystemName = appendHumanReadableNow(
      f["priceSystemName"],
      dateOverride,
    );
    ...
    componentProductName = appendHumanReadableNow(
      f["componentProductName"],
      dateOverride,
    );

    secondComponentProductName = appendHumanReadableNow(
      f["secondComponentProductName"],
      dateOverride,
    );
  ...
  it("Create plant", function () {
    ...
    cy.fixture("misc/misc_dictionary.json").then((miscDictionary) => {
      cy.selectInListField(
        ...
        getLanguageSpecific(miscDictionary, "S_Resource_MFG_Type_Plant"),
      );
    });
  ...
  it("Create 1st BOM's component product which is 2nd BOM's main product", function () {
    cy.fixture("product/simple_product.json").then((productJson) => {
      Object.assign(new Product(), productJson)
        ...
        .setSold(false)
        .setDescription(
          `Component of ${firstBomName}; main product of ${secondBomName}`,
        )
        .apply();
    });
    cy.getCurrentWindowRecordId().then((id) => {
    ...
    it("Create 2nd BOM", function () {
      cy.fixture("product/bill_of_material.json").then((billMaterialJson) => {
        Object.assign(new BillOfMaterial(), billMaterialJson)
          ...
          .addLine(
            new BillOfMaterialLine()
              .setProduct(secondComponentProductName)
              .setQuantity(10),
          )
          .apply();
      });
...
describe("Perform the actual test", function () {
  ...
  it("Expect one production order for 1st BOM's main product", function () {
    ...
    cy.expectNumberOfRows(1).selectNthRow(0).dblclick();

    cy.fixture("misc/misc_dictionary.json").then((miscDictionary) => {
      const draftedStatus = getLanguageSpecific(
        miscDictionary,
        DocumentStatusKey.Drafted,
      );
      cy.log(
        `Verify that the production order's doc status is ${draftedStatus}`,
      );
